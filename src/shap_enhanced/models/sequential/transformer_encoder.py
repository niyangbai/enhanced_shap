...
class TransformerEncoder(BaseModel):
    ...
    def __init__(self, input_size: int, hidden_size: int, output_size: int, num_layers: int = 1, num_heads: int = 4):
        """Initialize the Transformer Encoder model.

        :param int input_size: Number of input features.
        :param int hidden_size: Number of features in the hidden state (dimension of the model).
        :param int output_size: Number of output features.
        :param int num_layers: Number of layers in the transformer (default is 1).
        :param int num_heads: Number of attention heads (default is 4).
        """
        super().__init__()
    ...
    def fit(self, X: torch.Tensor, y: torch.Tensor, epochs: int = 20, batch_size: int = 32, lr: float = 0.001):
        """Train the Transformer Encoder model on input data X and target labels y."""
        ...
        for epoch in range(epochs):
            for i in range(0, len(X), batch_size):
                ...
                # Forward pass
                output = self.transformer_encoder(batch_x)

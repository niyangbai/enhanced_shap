...
class TabularXGBoost(BaseModel):
    """XGBoost model for Tabular Data."""

    def __init__(self, params: dict):
        """Initialize the XGBoost model.

        :param dict params: XGBoost hyperparameters.
        """
        super().__init__()
        self.model = xgb.XGBRegressor(**params)  # Use XGBClassifier for classification tasks
    ...
    def fit(self, X: torch.Tensor, y: torch.Tensor, eval_set: Optional[tuple] = None, 
        ...
        """Train the XGBoost model on input data X and target labels y."""
    ...
    def evaluate(self, X: torch.Tensor, y: torch.Tensor) -> float:
        """Evaluate the XGBoost model on input data X and labels y."""
        X, y = X.numpy(), y.numpy()

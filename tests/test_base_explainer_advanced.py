...
class ComplexMockExplainer(BaseExplainer):
    ...
    def shap_values(self, X, check_additivity=True, **kwargs):
        if isinstance(X, np.ndarray):
            ...
            result = [np.random.random(len(x)) * 0.1 if isinstance(x, (list, np.ndarray)) else 0.1 for x in X]
        else:
            result = np.array([0.1])
...
class TestBaseExplainerAdvanced(unittest.TestCase):
    ...
    def test_kwargs_passing(self):
        ...
        result = explainer.shap_values(np.array([1, 2]), custom_param=True, another_param="test")
